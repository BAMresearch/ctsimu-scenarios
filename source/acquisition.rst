...
.. code-block:: json-object
	...
	"scan_speed": {"value": 360, "unit": "deg/h", "uncertainty": 0, "uncertainty_unit": "deg/h"}
...
Beginning from the start angle, the necessary number of angular steps is performed (in the case of a stop&go scan). It is assumed that a frame is taken before each step (starting with the first frame at the start angle). The parameter :json:`"include_final_angle":` can be set to :json:`true` if the last projection should be taken after the stop angle has been reached.

.. code-block:: json-object
	:linenos:
	:lineno-start: 340

	"include_final_angle": true  false
